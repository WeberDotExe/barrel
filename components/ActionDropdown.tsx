        ...
        {["rename","delete","share"].includes(value)&& (
            <DialogFooter className='flex flex-col gap-3 md:flex-row'>
                ...
                <Button onClick={handleAction} className='modal-submit-button'>
                    ...
                    {isLoading && (<Image src='/assets/icons/loader.svg' width={24} height={24} alt='loader' className='animate-spin' />)}
                </Button>
            </DialogFooter>
        )}
      </DialogContent>)
  ...
  <DropdownMenuContent>
   ...
   {actionsDropdownItems.map((actionItem)=>(
    <DropdownMenuItem key={actionItem.value} className='shad-dropdown-item' onClick={()=>{
        ...
        {actionItem.value === "download" ? <Link href={constructDownloadUrl(file.bucketFileId)} download={file.name} className='flex item-center gap-2' >
            <Image src={actionItem.icon} width={30} height={30} alt={actionItem.label}  />
            {actionItem.label}
        </Link>: <div className='flex item-center gap-2' >
        <Image src={actionItem.icon} width={30} height={30} alt={actionItem.label}  />
        {actionItem.label}
        </div> }
    </DropdownMenuItem>
