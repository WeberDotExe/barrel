...
export const createAccount = async ({fullname,email}:{fullname:string;email:string}) => {
    ...
    if(!existingUser ){
        const {databases} = await createAdminClient();
        await databases.createDocument(appwriteConfig.databaseId,appwriteConfig.usersCollectionId,ID.unique(),{fullname,email,avatar:"/assets/icons/avatar.jpeg",accountId},);
